...
{
	_HA_ATOMIC_OR(&tg_ctx->threads_harmless, tid_bit);
	while (_HA_ATOMIC_LOAD(&tg_ctx->threads_want_rdv) & tg->threads_enabled & ~tid_bit) {
		ha_thread_relax();
	}
}
...
void thread_isolate()
{
	...
	_HA_ATOMIC_OR(&tg_ctx->threads_want_rdv, tid_bit);
	...
	while (1) {
		if (unlikely((old & tg->threads_enabled) != tg->threads_enabled))
			old = _HA_ATOMIC_LOAD(&tg_ctx->threads_harmless);
		else if (_HA_ATOMIC_CAS(&tg_ctx->threads_harmless, &old, old & ~tid_bit))
			break;
...
void thread_isolate_full()
{
	...
	_HA_ATOMIC_OR(&tg_ctx->threads_idle, tid_bit);
	_HA_ATOMIC_OR(&tg_ctx->threads_harmless, tid_bit);
	__ha_barrier_atomic_store();
	_HA_ATOMIC_OR(&tg_ctx->threads_want_rdv, tid_bit);
	...
	while (1) {
		...
		if (unlikely((old & tg->threads_enabled) != tg->threads_enabled))
			old = _HA_ATOMIC_LOAD(&tg_ctx->threads_harmless);
			 ...
			 _HA_ATOMIC_CAS(&tg_ctx->threads_harmless, &old, old & ~tid_bit))
			break;
	 ...
	 */
	_HA_ATOMIC_AND(&tg_ctx->threads_idle, ~tid_bit);
}
...
void thread_release()
{
	_HA_ATOMIC_AND(&tg_ctx->threads_want_rdv, ~tid_bit);
}
